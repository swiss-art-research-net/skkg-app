...
def synchroniseAndTranslate(*, inputDir, authKey):
    ...
    # Default file is the one without a language label
    defaultFile = [f for f in files if not re.search(r'_(.*)\.properties', f)][0]
    ...
    translator = deepl.DeepLClient(authKey)
    for file in fields:
            ...
            for term in tqdm(allTerms):
                if fields[file][term]:
                    ...
                    if lang:
                        ...
                        print("No language label found for file %s, skipping translation" % file)
                        print("Term: %s" % term )
